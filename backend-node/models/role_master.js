...
const RoleMasterSchema = new mongoose.Schema(
  {
    dept_id: { type: Number, required: true, index: true },
    department: { type: String, required: true, trim: true },
    dept_page_link: { type: String, trim: true, default: '' },
    dept_head_email: { type: String, trim: true, default: '' },
    dept_group_email: { type: String, trim: true, default: '' },
    parent_department: { type: String, trim: true, default: '' },
    department_type: { type: String, enum: [...DEPARTMENT_TYPES, ''], default: '' },
    department_head: { type: String, trim: true, default: '' },
    department_deputy: { type: String, trim: true, default: '' },
    desig_id: { type: Number, required: true, index: true },
    designation: { type: String, required: true, trim: true },
    emp_id: { type: String, trim: true, default: '', index: true },
    emp_name: { type: String, trim: true, default: '' },
    desig_email_id: { type: String, trim: true, default: '' },
    role_document_link: { type: String, trim: true, default: '' },
    jd_link: { type: String, trim: true, default: '' },
    remarks: { type: String, trim: true, default: '' },
    management_level: { type: String, trim: true, default: '', index: true },
    reporting_manager: { type: String, trim: true, default: '' },
  },
  { collection: ROLE_MASTER_COLLECTION, timestamps: true }
);

RoleMasterSchema.index(
  { dept_id: 1, desig_id: 1 },
  { unique: true, partialFilterExpression: { dept_id: { $exists: true, $ne: null }, desig_id: { $exists: true, $ne: null } } }
);
...
RoleMasterSchema.statics.getUniqueDepartments = function () {
  return this.aggregate([
    { $group: { _id: '$dept_id', dept_id: { $first: '$dept_id' }, department: { $first: '$department' } } },
    { $sort: { department: 1 } },
...
RoleMasterSchema.statics.getUniqueDesignations = function (department) {
  ...
  pipeline.push(
    { $group: { _id: '$desig_id', desig_id: { $first: '$desig_id' }, designation: { $first: '$designation' } } },
    { $sort: { designation: 1 } },
