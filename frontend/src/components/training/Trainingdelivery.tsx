...
export default function TrainingDelivery() {
  ...
  const getTargetAudienceText = (audience: CompletedTraining['targetAudience']) => {
    switch (audience.type) {
      case 'all':         return 'All Employees';
      case 'departments': return `${audience.departments?.length || 0} Department(s)`;
      case 'levels':      return `Level ${audience.levels?.join(', ') || ''}`;
      case 'roles':       return `${audience.roles?.length || 0} Role(s)`;
    ...
    <div className="p-6">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-sm mt-1">Completed training sessions will appear here.</p>
        </div>
      ) : (
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filtered.map(t => (
                <tr key={t._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-4 text-sm text-gray-500">{getTargetAudienceText(t.targetAudience)}</td>
                  <td className="px-5 py-4">
                    <button
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-900">{viewingTraining.capabilityArea}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase mb-1">Capability Skill</p>
                  <p className="text-sm text-gray-900">{viewingTraining.capabilitySkill}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-900">{viewingTraining.trainerName}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase mb-1">Type</p>
                  <span className={`px-2 py-1 text-xs rounded-full ${getTypeColor(viewingTraining.type)}`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase mb-1">Training Date</p>
                  <p className="text-sm text-gray-900">{new Date(viewingTraining.trainingDate).toLocaleDateString()}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase mb-1">Time</p>
                  <p className="text-sm text-gray-900">{viewingTraining.startTime} – {viewingTraining.endTime}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-900">{viewingTraining.venue || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase mb-1">Online Link</p>
                  {viewingTraining.onlineLink ? (
                    <a href={viewingTraining.onlineLink} target="_blank" rel="noopener noreferrer"
                      className="text-blue-600 hover:text-blue-800 text-sm">Join Online</a>
                  ) : (
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-900">{getTargetAudienceText(viewingTraining.targetAudience)}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase mb-1">Max Attempts</p>
                  <p className="text-sm text-gray-900">{viewingTraining.maxAttempts}</p>
                </div>
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase mb-1">Feedback Window</p>
                  <p className="text-sm text-gray-900">{viewingTraining.feedbackWindow}h</p>
                </div>
              </div>
              ...
              {(viewingTraining.contentPdfLink || viewingTraining.videoLink || viewingTraining.assessmentLink) && (
                <div>
                  <p className="text-xs font-medium text-gray-500 uppercase mb-2">Content Links</p>
                  <div className="flex flex-wrap gap-3">
                    {viewingTraining.contentPdfLink && (
                      <a href={viewingTraining.contentPdfLink} target="_blank" rel="noopener noreferrer"
                    ...
                    {viewingTraining.videoLink && (
                      <a href={viewingTraining.videoLink} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-1.5 text-blue-600 hover:text-blue-800 text-sm">
                        <Video className="w-4 h-4" /> Video
                      </a>
                    )}
                    {viewingTraining.assessmentLink && (
                      <a href={viewingTraining.assessmentLink} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-1.5 text-blue-600 hover:text-blue-800 text-sm">
              ...
              <div className="grid grid-cols-2 gap-4 pt-2 border-t border-gray-100">
                <div>
                  ...
                  <p className="text-sm text-gray-900">{viewingTraining.createdBy}</p>
                </div>
                {viewingTraining.approvedBy && (
                  <div>
                    <p className="text-xs font-medium text-gray-500 uppercase mb-1">Approved By</p>
                    <p className="text-sm text-gray-900">{viewingTraining.approvedBy}</p>
                  </div>
                )}
              </div>
            </div>
