...
const OnboardingDashboard: React.FC = () => {
  ...
  // "Current employee" = has actually joined and hasn't exited. Matches the
  // definition Salary Revision, Confirmations, and the Configuration
  // employee picker all use (joiningStatus === "Joined" + not exited) —
  // stricter than just "not exited", since a candidate who backed out
  // before starting ("Not Joining") or hasn't started yet ("Yet To Join
  // Office") is neither, and would otherwise silently drift this stat out
  // of sync with those other pages the moment one shows up.
  const isCurrent = (r: OnboardingRow) =>
    r.joiningStatus === "Joined" && !isExited(r);
