...
const API_BASE = 'http://3.110.162.1:5000/api'; // change to production later
...
export default function NewRequisitionForm() {
  ...
  // Fetch employees, departments from role master
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const [empRes, roleMasterRes] = await Promise.all([

          fetch(`${API_BASE}/employees/`),

          fetch(`${API_BASE}/role-master/`),

        ]);



        if (!empRes.ok || !roleMasterRes.ok) {
        ...
        const roleMasterData = await roleMasterRes.json();
        ...
        const roleMasterList = Array.isArray(roleMasterData) ? roleMasterData : (roleMasterData.data || []);



        // Extract unique departments from role master

        const uniqueDepartments = Array.from(

          new Map(roleMasterList.map((role: any) => [role.department, { 
        ...
        setDesignations(roleMasterList); // Store all role master data for designations

        setFilteredDesignations([]); // Initialize filteredDesignations state

      } catch (err) {

        console.error('Fetch error:', err);

        setError('Failed to load employees/role master data');
  ...
  // Filter designations when department changes (from role master data)
  ...
  useEffect(() => {
    ...
    // Filter designations from role master data based on selected department

    const filteredDesignations = designations.filter((role: any) => 

      role.department === hiringDept

    );

    

    // Update the designations state with filtered results

    setFilteredDesignations(filteredDesignations);

  }, [hiringDept, designations]);
