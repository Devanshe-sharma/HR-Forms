import { useState, useEffect, useRef, useCallback } from 'react';  // ← useCallback added
import {
  Box,
  Typography,
  Stack,
    ...
    <Box>
      ...
      {uploading && (
        <LinearProgress
          ...
          value={progress}
          sx={{
            mt: 0.6, borderRadius: 4, height: 3, bgcolor: `${accentColor}15`,
...
function LinkEditor({
  ...
  if (!editing) return (
    <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap" rowGap={0.8}>
      ...
      {isHR && (
        <FileUploadButton accentColor={accentColor} onUploaded={handleUploaded} uploadPath={uploadPath} />
      )}
    </Stack>
  );
    ...
    <Stack spacing={0.8} sx={{ p: 1.5, bgcolor: 'rgba(255,255,255,0.95)', borderRadius: '10px', border: '1px solid #E5E7EB', mt: 0.8 }}>
      ...
      <Stack direction="row" spacing={0.8} alignItems="center">
        <Button
          ...
          sx={{
            bgcolor: accentColor, borderRadius: '7px', textTransform: 'none', fontSize: '0.78rem', px: 2,
...
// ── LinkDisplay ───────────────────────────────────────────────────────────────
function LinkDisplay({ link, accentColor, systemName }: {
...
function DeptPresentations({ dept, dd, c, onUpdate }: {
  ...
  const addReview = async () => {
    ...
    setSaving(true);
    const r = await api.post<QuarterPPT>(`/dept-orientation/${dd?.id}/review-ppts`, { ...newP, name: sysName });
    ...
    <Box>
      ...
      {tab === 0 && (
        <Stack spacing={1.2}>
          ...
          <LinkEditor currentUrl={dd?.onboardingPPT?.url || ''} systemName={onboardingSystemName} accentColor={c} onSave={saveOnboarding} uploadPath={`/dept-orientation/${dd?.id}/upload`} placeholder="Google Drive PPT link" />
        </Stack>
      )}
            ...
            <DialogContent>
              <Stack spacing={2} mt={0.5}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <FileUploadButton accentColor={c} onUploaded={url => setNewP(p => ({ ...p, url }))} uploadPath={`/dept-orientation/${dd?.id}/upload`} />
                </Box>
              </Stack>
            </DialogContent>
      ...
      {tab === 2 && (
        <Stack spacing={1.2}>
          ...
          <LinkEditor currentUrl={dd?.masterPPT?.url || ''} systemName={masterSystemName} accentColor={c} onSave={saveMaster} uploadPath={`/dept-orientation/${dd?.id}/upload`} placeholder="Google Drive Master PPT link" />
        </Stack>
      )}
    </Box>
  );
...
// FIX: onRefresh replaces window.location.reload()
// ════════════════════════════════════════════════════════════════════════════
function JDRoleDocs({ dept, c, designations, onRefresh }: {
  dept: string;
  c: string;
  designations: Designation[];
  onRefresh: () => void;  // ← FIX: triggers re-fetch instead of page reload
}) {
  ...
  }>({ open: false, type: 'jd', designation: null });
  const [uploading, setUploading] = useState(false);
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      if (response.success) {
        onRefresh();  // ← FIX: was window.location.reload()
      }
    } catch (error) {
  ...
  return (
    <Box>
                      ...
                      <Box sx={{ p: 1.5, bgcolor: 'white', borderRadius: '10px', border: `1px solid ${c}20` }}>
                        ...
                        {hasJD ? (
                          <Stack direction="row" spacing={1} alignItems="center">
                            ...
                            {isHR && (
                              <Tooltip title="Upload New JD">
                                <IconButton onClick={() => setUploadDialog({ open: true, type: 'jd', designation: r })}
                          ...
                          <Stack direction="row" spacing={1} alignItems="center">
                            ...
                            {isHR && (
                              <Button size="small" startIcon={<UploadIcon sx={{ fontSize: 14 }} />}
                          ...
                          <Stack direction="row" spacing={1} alignItems="center">
                            ...
                            {isHR && (
                              <Tooltip title="Upload New Role Doc">
                                <IconButton onClick={() => setUploadDialog({ open: true, type: 'role_doc', designation: r })}
      ...
      <Dialog open={uploadDialog.open} onClose={() => setUploadDialog({ open: false, type: 'jd', designation: null })} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '16px' } }}>
        ...
        <DialogContent>
          <Stack spacing={2} mt={0.5}>
            ...
            <Stack direction="row" spacing={1}>
              <Button variant={uploadType === 'file' ? 'contained' : 'outlined'} size="small" onClick={() => setUploadType('file')}
                sx={{ borderRadius: '8px', textTransform: 'none', bgcolor: uploadType === 'file' ? c : 'transparent', color: uploadType === 'file' ? 'white' : c, borderColor: c }}>
                Upload File
              </Button>
              <Button variant={uploadType === 'drive' ? 'contained' : 'outlined'} size="small" onClick={() => setUploadType('drive')}
                sx={{ borderRadius: '8px', textTransform: 'none', bgcolor: uploadType === 'drive' ? c : 'transparent', color: uploadType === 'drive' ? 'white' : c, borderColor: c }}>
                Google Drive Link
              </Button>
            </Stack>
            {uploadType === 'file' ? (
              <>
                <TextField size="small" label="Select File" fullWidth value={selectedFile?.name || ''} onClick={() => fileInputRef.current?.click()}
                  InputProps={{ readOnly: true, startAdornment: <InputAdornment position="start"><UploadIcon sx={{ fontSize: 18, color: '#6B7280' }} /></InputAdornment> }}
                  sx={{ cursor: 'pointer', '& .MuiOutlinedInput-root': { borderRadius: '10px' }, '&:hover .MuiOutlinedInput-root': { borderColor: c } }}
                />
                <input ref={fileInputRef} type="file" hidden accept=".pdf,.doc,.docx" onChange={e => setSelectedFile(e.target.files?.[0] || null)} />
                {selectedFile && (
                  <Box sx={{ p: 1.5, bgcolor: '#F8FAFC', borderRadius: '8px', border: '1px solid #E2E8F0' }}>
                    <Typography fontSize="0.78rem" color="#64748B">Selected: <span style={{ fontWeight: 600, color: '#334155' }}>{selectedFile.name}</span></Typography>
                    <Typography fontSize="0.72rem" color="#94A3B8">Size: {(selectedFile.size / 1024 / 1024).toFixed(2)} MB</Typography>
                  </Box>
                )}
              </>
            ) : (
              <TextField size="small" label="Google Drive Link" fullWidth value={driveLink} onChange={e => setDriveLink(e.target.value)} placeholder="https://drive.google.com/file/d/..."
                InputProps={{ startAdornment: <InputAdornment position="start"><LinkIcon sx={{ fontSize: 18, color: '#6B7280' }} /></InputAdornment> }}
        ...
        <DialogActions sx={{ px: 3, pb: 2.5 }}>
          <Button onClick={() => setUploadDialog({ open: false, type: 'jd', designation: null })} sx={{ textTransform: 'none', color: '#6B7280', borderRadius: '8px' }}>Cancel</Button>
          <Button variant="contained" onClick={handleFileUpload} disabled={uploading || (uploadType === 'file' ? !selectedFile : !driveLink)}
...
// FIX: sends JSON instead of FormData; supplies required `content` field
// ════════════════════════════════════════════════════════════════════════════
function DeptNotes({ dept, c, dd, onUpdate }: {
  ...
  const addNote = async () => {
    ...
    // FIX: backend route expects JSON { title, content } — content is required by NoteSchema
    // Store the link or filename as the content value so the schema validation passes
    const title = generateNoteSystemName();
    const content = link || (selectedFile ? selectedFile.name : '');

    try {
      const r = await api.post<DeptNote>(`/dept-orientation/${dd?.id}/notes`, { title, content });
      if (r.success && r.data) {
        onUpdate(dept, { notes: [...(dd?.notes || []), r.data] });
      }
    } catch (err) {
    ...
    <Box>
                ...
                <Box sx={{ px: 2.5, py: 2, bgcolor: '#FAFAFA', borderTop: `1px solid ${c}12` }}>
                  {note.link ? (
                    <Stack spacing={1.5}>
                      ...
                      <Button href={note.link} target="_blank" startIcon={<OpenInNewIcon sx={{ fontSize: 14 }} />}
                    ...
                    <Stack spacing={1.5}>
                      ...
                      <Stack direction="row" spacing={1} alignItems="center">
                        <Button href={note.attachment} target="_blank" startIcon={<OpenInNewIcon sx={{ fontSize: 14 }} />}
                          sx={{ textTransform: 'none', fontWeight: 600, fontSize: '0.82rem', color: 'white', bgcolor: c, borderRadius: '8px', px: 2, py: 0.7, '&:hover': { bgcolor: c, opacity: 0.88 } }}>
                          Open File
                        </Button>
      ...
      <Dialog open={addDlg} onClose={() => setAddDlg(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: '16px' } }}>
        ...
        <DialogContent>
          <Stack spacing={2} mt={0.5}>
            <Box sx={{ p: 1.5, bgcolor: '#F0F9FF', borderRadius: '8px', border: '1px solid #BAE6FD' }}>
              <Typography fontSize="0.75rem" color="#0369A1" fontWeight={600} mb={0.5}>Auto-generated Name:</Typography>
              <Typography fontSize="0.82rem" color="#1E40AF" fontFamily="monospace">{generateNoteSystemName()}</Typography>
            </Box>
            <TextField size="small" label="Link (Optional)" fullWidth value={link} onChange={e => setLink(e.target.value)}
              placeholder="https://example.com/resource"
              sx={{ '& .MuiOutlinedInput-root': { borderRadius: '10px', fontSize: '0.85rem' } }}
            />
            <Typography textAlign="center" sx={{ fontSize: '0.78rem', color: '#6B7280', my: 1 }}>OR</Typography>
            <TextField size="small" label="Upload File (Optional)" fullWidth value={selectedFile?.name || ''} onClick={() => fileInputRef.current?.click()}
              InputProps={{ readOnly: true, startAdornment: <InputAdornment position="start"><UploadIcon sx={{ fontSize: 18, color: '#6B7280' }} /></InputAdornment> }}
              sx={{ cursor: 'pointer', '& .MuiOutlinedInput-root': { borderRadius: '10px' }, '&:hover .MuiOutlinedInput-root': { borderColor: c } }}
            />
            <input ref={fileInputRef} type="file" hidden accept=".pdf,.doc,.docx,.txt,.jpg,.jpeg,.png" onChange={e => setSelectedFile(e.target.files?.[0] || null)} />
            {selectedFile && (
              <Box sx={{ p: 1.5, bgcolor: '#F8FAFC', borderRadius: '8px', border: '1px solid #E2E8F0' }}>
                <Typography fontSize="0.78rem" color="#64748B">File: <span style={{ fontWeight: 600, color: '#334155' }}>{selectedFile.name}</span></Typography>
                <Typography fontSize="0.72rem" color="#94A3B8">Size: {(selectedFile.size / 1024 / 1024).toFixed(2)} MB</Typography>
              </Box>
            )}
          </Stack>
        ...
        <DialogActions sx={{ px: 3, pb: 2.5 }}>
          <Button onClick={() => setAddDlg(false)} sx={{ textTransform: 'none', color: '#6B7280', borderRadius: '8px' }}>Cancel</Button>
          <Button variant="contained" onClick={addNote} disabled={saving || (!link && !selectedFile)} sx={{ bgcolor: c, borderRadius: '8px', textTransform: 'none', px: 3 }}>
  ...
  return (
    <Stack spacing={1.5}>
      <Box sx={{ p: 2, borderRadius: '12px', bgcolor: 'white', border: `1px solid ${c}20`, boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
        ...
        <Box mt={1.2}>
          <LinkEditor currentUrl={dd?.recruitmentTest?.url || ''} systemName={recSystemName} accentColor={c} onSave={saveRec} uploadPath={`/dept-orientation/${dd?.id}/upload`} />
        </Box>
      </Box>
      ...
      <Box sx={{ p: 2, borderRadius: '12px', bgcolor: 'white', border: '1px solid #10B98120', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
        ...
        <Box mt={1.2}>
          <LinkEditor currentUrl={dd?.onboardingTest?.url || ''} systemName={onbSystemName} accentColor="#10B981" onSave={saveOnb} uploadPath={`/dept-orientation/${dd?.id}/upload`} />
        </Box>
      </Box>
    </Stack>
  );
...
function NotionDeptSelect({ departments, value, onChange }: {
  departments: string[];
  value: string;
  ...
  return (
    <Box sx={{ position: 'relative' }}>
      <Box onClick={() => setOpen(o => !o)} sx={{
        ...
        <ArrowDownIcon sx={{ fontSize: 16, color: activeStyle.color, opacity: 0.7, transform: open ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} />
      </Box>
      ...
      <Collapse in={open} timeout={160}>
        <Box sx={{
          ...
          boxShadow: '0 8px 32px rgba(0,0,0,0.12)', py: 0.6,
        }}>
          {departments.map(d => {
            ...
            return (
              <Box key={d} onClick={() => { onChange(d); setOpen(false); }}
                sx={{ display: 'flex', alignItems: 'center', gap: 1.2, px: 1.5, py: 0.9, cursor: 'pointer', bgcolor: isActive ? '#F8FAFC' : 'transparent', '&:hover': { bgcolor: '#F8FAFC' } }}>
                <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.7, px: 1, py: 0.25, bgcolor: s.bg, border: `1px solid ${s.border}`, borderRadius: '6px' }}>
...
export default function DeptOrientationPage() {
  ...
  // FIX: extracted into useCallback so it can be passed as onRefresh to JDRoleDocs
          ...
          {loading ? (
            ...
            <Stack spacing={2}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, px: 2.5, py: 1.8, bgcolor: 'white', borderRadius: '14px', border: '1px solid #E5E7EB', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
                ...
                <NotionDeptSelect departments={departments} value={activeDept} onChange={setActiveDept} />
              </Box>
              ...
              <SectionCard num={2} label="JD & Role Documents" icon={<WorkIcon sx={{ fontSize: 21 }} />}>
                {/* FIX: onRefresh={fetchData} was missing — caused JD uploads to either
                    reload the whole page or silently fail with no state update */}
                <JDRoleDocs dept={activeDept} c={deptColor} designations={designations} onRefresh={fetchData} />
              </SectionCard>
